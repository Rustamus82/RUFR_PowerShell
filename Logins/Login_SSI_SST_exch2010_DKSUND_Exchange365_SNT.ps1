...
$Global:UserCredSST = Get-Credential "sst.dk\adm-snt" -Message "SST AD login og import af AD modulet"
...
#Get-Credential "adm-snt@dksund.dk" | Export-Clixml C:\RUFR_PowerShell\Logins\xml\rufr_o365.xml
...
$Global:UserCredDksund = Get-Credential "adm-snt@dksund.dk" -Message "DKSUND AD login, Exchange Online & Hybrid"
#Connect-ExchangeOnline -UserPrincipalName "adm-snt@dksund.dk" -ShowProgress $true -ShowBanner:$false
#Connect-ExchangeOnline -Credential $Global:UserCredDksund -ShowProgress $true -ShowBanner:$false
#Connect-ExchangeOnline -UserPrincipalName "adm-snt@dksund.dk" -ShowProgress $true 
Connect-AzureAD -AccountId "adm-snt@dksund.dk"
...
$Global:UserCredSSI = Get-Credential "ssi\adm-snt" -Message "SSI AD login"
...
if (-not(Get-PSDrive 'SSIAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "ssi\adm-snt") 
...
if (-not(Get-PSDrive 'SSTAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "adm-snt@dksund.dk") 
