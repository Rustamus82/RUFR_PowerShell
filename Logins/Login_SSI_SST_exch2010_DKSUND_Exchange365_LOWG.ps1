...
$Global:UserCredSST = Get-Credential "sst.dk\adm_lowg" -Message "SST AD login og import af AD modulet"
...
#Get-Credential "adm_lowg@dksund.dk" | Export-Clixml C:\RUFR_PowerShell\Logins\xml\rufr_o365.xml
...
$Global:UserCredDksund = Get-Credential "adm_lowg@dksund.dk" -Message "DKSUND AD login, Exchange Online & Hybrid"
Connect-ExchangeOnline -UserPrincipalName "adm_lowg@dksund.dk" -ShowProgress $true -ShowBanner:$false
#Connect-ExchangeOnline -Credential $Global:UserCredDksund -ShowProgress $true -ShowBanner:$false
#Connect-ExchangeOnline -UserPrincipalName "adm_lowg@dksund.dk" -ShowProgress $true 
Connect-AzureAD -AccountId "adm_lowg@dksund.dk"
...
if (-not(Get-PSDrive 'SSIAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "ssi\adm_lowg") 
...
if (-not(Get-PSDrive 'DKSUNDAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "sst.dk\adm_lowg"') 
...
if (-not(Get-PSDrive 'SSTAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "adm_lowg@dksund.dk") 
