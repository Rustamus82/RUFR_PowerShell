...
$Global:UserCredSST = Get-Credential "sst.dk\adm_krle" -Message "SST AD login og import af AD modulet"
...
#Get-Credential "adm_krle@dksund.dk" | Export-Clixml C:\RUFR_PowerShell\Logins\xml\rufr_o365.xml
...
$Global:UserCredDksund = Get-Credential "adm_krle@dksund.dk" -Message "DKSUND AD login, Exchange Online & Hybrid"
Connect-ExchangeOnline -UserPrincipalName "adm_krle@dksund.dk" -ShowProgress $true -ShowBanner:$false
#Connect-ExchangeOnline -Credential $Global:UserCredDksund -ShowProgress $true -ShowBanner:$false
#Connect-ExchangeOnline -UserPrincipalName "adm_krle@dksund.dk" -ShowProgress $true 
Connect-AzureAD -AccountId "adm_krle@dksund.dk"
...
$Global:UserCredSSI = Get-Credential "ssi\adm_krle" -Message "SSI AD login"
...
if (-not(Get-PSDrive 'SSIAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "ssi\adm_krle") 
...
if (-not(Get-PSDrive 'DKSUNDAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "sst.dk\adm_krle"') 
...
if (-not(Get-PSDrive 'SSTAD' -ErrorAction SilentlyContinue)) {
    ...
    #alternativet creds: Credential $(Get-Credential -Message 'Enter Password' -UserName "adm_krle@dksund.dk") 
