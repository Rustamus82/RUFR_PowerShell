...
#"Reconnecting to Exchange 2016 SST" -foregroundcolor Cyan
try {
        ...
        $Global:SessionExchangeSST = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://s-exc-mbx02-p/PowerShell/ -Authentication Kerberos -SessionOption $Global:PSSessionOption -Credential $Global:UserCredDksund
        ...
        Write-Host "Reconnecting to Exchange 2016 SST" -foregroundcolor Cyan 
        Get-PSSession  | Where-Object{$_.ComputerName -like "s-exc-mbx0*"} | Remove-PSSession -ErrorAction SilentlyContinue
        $Global:SessionExchangeSST = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://s-exc-mbx03-p/PowerShell/ -Authentication Kerberos -SessionOption $Global:PSSessionOption -Credential $Global:UserCredDksund
