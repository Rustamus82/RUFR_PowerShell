...
catch [System.Exception]
{
    ...
    Write-Host
    Write-Host "    [Cause nr 1] Probably Reinstall_VM_script_AsAdmin.bat not startet as Admin. " -ForegroundColor Yellow
    Write-Host "    [Solution nr 1] Please right click on the Reinstall_VM_script_AsAdmin.bat and choose ad Administrator. " -ForegroundColor Green
    Write-Host " or"
    Write-Host "    [Cause nr 1] on server core start as [start-process PowerShell -verb runas] then execute Reinstall_VM_script_AsAdmin.ps1 " -ForegroundColor Yellow
     Write-Host
    Write-Host
