<# 
.SYNOPSIS 
Skifter brugerens kodeord fra -Glpw til -Nypw 
Script opdateret Mikael Veistrup-Vetlov @ 20210909
Script oprettet Mikael Veistrup-Vetlov @ 20190522

.Parameter Oldpw
Angiv gammelt kodeord for at skifte password for aktuel bruger, i dette domain.
Der er ingen kontrol af validitet af kodeordet.

.Parameter Newpw
Angiv Nyt kodeord for at skifte password for aktuel bruger, i dette domain.
Der er ingen kontrol af validitet af kodeordet.

.Parameter Ok
Angiv j for at undgå kontrolspørgsmål om skift af password for aktuel bruger, i dette domain.

.DESCRIPTION 
Skifter brugerens kodeord fra -Glpw til -Nypw 

.EXAMPLE 
.\Skift-Pw.ps1 -Glpw JegErForGammel¤0 -Nypw £3NuNogetHeltNyt

.EXAMPLE 
.\Skift-Pw.ps1 -Oldpw glkodeord -Newpw nytKodeord
...
#> 
    ...
    param( 
		...
		HelpMessage="Angiv kodeord der skal skiftes fra.")] 
        [Alias("Glpw")] 
        [string] $Oldpw 
        ,[Parameter(Mandatory, Position=1, ValueFromPipelineByPropertyName,
		HelpMessage="Angiv kodeord der skal skiftes til.")] 
        [Alias("Nypw")] 
		[string]$Newpw
	...
	Begin{
		$Da="DKSUND.DK","SSI.AD","SST.DK","Ikke Valgt","intdmz.dk","pubdmz.dk","ssidmz01.local","imkit.dk","nsidsdn.dk","DKSUNDTEST.DK","Ikke Valgt"
		$Ds ="S-AD-DC-02P.dksund.dk","SRV-AD-DC06.ssi.ad","s-dc21-p.sst.dk","Ikke Valgt","idmzdc03.intdmz.dk","s-pdmzdc03-p.pubdmz.dk","srv-ad-dmzdc01.ssidmz01.local","S-IMKIT-DC-01P.IMKIT.DK","srv-ad-dc02.nsidsdn.dk","s-ad-dc-01t.dksundtest.dk","Ikke Valgt"
		$b=$env:USERName
		$dom=$env:USERDNSDOMAIN
		For($i=0;$i -lt $da.count;$i++){
			If ($dom -eq $Da[$i]){$s=$Ds[$i];Break}
		}
		If($i -ge $da.count){"Wrong Domain, not found :-( ";exit}
		$oldpws = $oldpw | ConvertTo-SecureString -asPlainText -Force
		$newpws = $newpw | ConvertTo-SecureString -asPlainText -Force
		If($Ok -ne "j") {
			$rc=Read-Host "Skift Bruger $b kodeord fra $oldpw til $Newpw ?"
			If(!($rc -in 'j','Ja','Y','yes')){Write-Host "Nå ok, ingen action, Farvel :-)";exit}
		}
		Set-ADAccountPassword -identity $b -OldPassword $oldpws -NewPassword $newpws -server $s #-Credential $AdminCredentials
	}
# SIG # Begin signature block
# MIIdAgYJKoZIhvcNAQcCoIIc8zCCHO8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD35QJas2133JjX
# izEjb69vS6oWeZoekN6JMtEp11m70qCCGBMwggT+MIID5qADAgECAhANQkrgvjqI
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIKeG8qjDU0uit2GXpPxdotxb
# efXsZ7+xZQlDFoQ2u/kvMA0GCSqGSIb3DQEBAQUABIIBAE0LFzey/k4OH/1QxI+w
# s5bnNDyd/IbnFXCYflfwQwWi6CGbvcEBMb7Ng0ZRGxxPisT5AIrp8OM+Oo6gYETA
# OgQHwJwBt6QnJzJy4xYWOgtsZq168N/Nvwb1eGLiv3RDjj2skIEr7RoIrkdRxs4Z
# ZmlziZ2sA8TV7KjPAeiem0fssQKf35X0/jzlLomS0POK8SWHuWOdwOE017AUIzj+
# 2JWiqXehCsXaFRC/XROEaASs9vtGAl2ricwhtoGs02a2cCO8YjxraseTyJGLpskU
# uoXFy1QF/2/DAVkC8CgnbNfEDGtlWQEukLaTsVQ32q8WZqNyOl22ygA+2IC8At9Z
# DEqhggIwMIICLAYJKoZIhvcNAQkGMYICHTCCAhkCAQEwgYYwcjELMAkGA1UEBhMC
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDkxMzE0NTkz
# OVowLwYJKoZIhvcNAQkEMSIEIOSNlI8zdo36KzCJaM+fh27o9wTSNSBCDCPkm+Ys
# JoWRMA0GCSqGSIb3DQEBAQUABIIBABp3sURgUvFCdn+LayM8VemhgJZ2cQEevMsh
# 1OqFRb/X05KWDycrQzKoBHgRGRVoCN4wc+es7cgw/mW4jZqj6oKVmlzzYTanwap5
# nFlXy5pQq4FL7fV520i4DHMK28CcWigBbzUOrA1fltiuoyS8UcswN+wksDcAQ3xz
# WO2H/xZmlihPRK1QBii9U+mr7hom/DW5hOmpgY+PceAF8i3jAsG8L/mQx7aHZhL0
# FlAzRKewZ3nhkwG5TcymLXPbupG6tXm62dOouxL0LF4bCS4Cft1bERqIlRZW1fAd
# er0/5ZEsNOdwy0dwfHkclOuBDjsmb7S3hdQwi07J5zzvIBxTJmA=
