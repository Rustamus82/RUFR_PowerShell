...
Tilføjet mulighed for at publicere scriptet til anden lokation.

.Parameter File 
sign-doc.ps1 bruger  <-file> for at angive navnet på et script, der skal signeres.

.Parameter Publicer 
sign-doc.ps1 bruger argument <-Publicer> for at angive hvor det signerede script skal kopieres hen efter det er signeret.
SKAL være i formatet: x:\folder
standard værdien er ingen publicering.
...
#> 
	...
	Begin{
		...
		If (!($c.subject.count -gt 0)){
			Write-Error "Beklager der er ikke oprettet nogen certifikater via certmgr /s my  Opret et først."
			Exit
		}
