<# 
.SYNOPSIS 
Find-Log4j.ps1 søger c:-disk igennem, ignorerer fejl og finder filer der indeholder 'Log4J' eller 'slf4j'
Script opdateret Mikael Veistrup-Vetlov @ 20220107
Script oprettet Mikael Veistrup-Vetlov @ 20220104

.Parameter drev 
Find-Log4j.ps1 kan vælge at checke andet <-drev> end c: hvis ønsket -drev f

.Parameter java 
Find-Log4j.ps1 kan vælge at checke om der er installeret Java på server, med parameter -java j

.DESCRIPTION 
Find-Log4j.ps1 bruger søgeargumenterne 'Log4J' og 'slf4j' for at søge på c:-drev efter filer hvor disse indeholdes i filnavn / sti 
Filerne bliver vist i popup vindue, og man kan vælge at omdøbe, slette eller Liste filerne
Listen kan pipes videre til fil for dokumentation i en lokal fil, via '>'
Hvis man vælger at liste, kan man åbne filen (notepad) og kopiere de forekomster man vil omdøbe (til powershell)

.EXAMPLE 
.\Find-Log4j.ps1 > .\log4j\$env:COMPUTERNAME.log 
Søger c:-drev igennem viser forekomster, spørger om filer skal omdøbes, slettes eller Listes.

.EXAMPLE 
.\Find-Log4j.ps1 -drev f -java j > .\log4j\$env:COMPUTERNAME.log 
Checker om der er installeret program: *java* +
Søger f:-drev igennem, viser forekomster af  'Log4J' eller 'slf4j', spørger om filer skal omdøbes, slettes eller Listes.
...
#> 
    ...
    param( 
		...
		HelpMessage="Angiv Drevbogstav der skal checkes for  'Log4J' eller 'slf4j' ")] 
        [string] $drev="c"
        ,[Parameter(Mandatory=$false, Position=1, ValueFromPipelineByPropertyName,
		HelpMessage="Angiv j for at checke om der er installeret Java")] 
        [string] $Java="n"
		)
Begin{
	if(!($drev.length -gt 0)){$drev="c:"}else {$drev=$drev.substring(0,1)+":"}
	If (test-path $drev){Write-host "Drev: $drev fundet, fortsætter."}Else{Write-host "Drev: $drev ikke fundet, farvel!";Exit}
	$Global:EgoIsAdm=[bool](([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -match "S-1-5-32-544")
	If(!($EgoIsAdm)) {
		Write-Output "Du skal køre powershell som Administrator for at kunne slette / omdøbe låste filer på Server"
		Exit
	}
	$Global:EgoCrLf="`r`n" 
	$repl="l4jrepl"
	...
	$retur=@()
	$core=@()
	if(($Java.length -gt 0) -and ($Java.substring(0,1) -eq "j")){
		$Javaf=0
		$Prg=Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Select-Object DisplayName, DisplayVersion, Publisher, InstallDate 
		foreach ($p in $Prg){
			If(($p.DisplayName -match("java")) -or ($p.DisplayName -match("OpenJDK"))){
				$Javaf+=1;
				$retur+="#Java / OpenJDK Fundet: "+$p.DisplayName +" version: "+$p.DisplayVersion +" fra: "+$p.Publisher
			}
		}
		If($Javaf -lt 1) {$retur+="Java ikke fundet!"}Else{Write-host "Java fundet."}
	}
	$res=Get-ChildItem $drev\ -Recurse -ErrorAction SilentlyContinue|Where-Object {$_.fullname -match("$rm1") -or $_.fullname -match("$rm2")}| % { $_.FullName }
	If ($res.count -gt 0){
		$retur+="Der er fundet "+ $res.count + " Forekomster af filnavn '$rm1' / '$rm2' ! på drev $drev"
		foreach($r in $res){ if ($r -match("core")){$core+=$r}}
		if ($core.count -gt 0){$retur+="Heraf "+$core.count +" Core fundet."; $retur+=$core;$retur+="____________"}
		#Header
		$mHead="Der er fundet "+ $res.count + " Forekomster af filnavn '$rm1' / '$rm2' ! på drev $drev."
		#Body / Question
		$Lix=0
		$Lil=0
		#$r=$res[$Lix]
		foreach($r in $res){
	#Write-Host "lix: $Lix lil: $lil "  
			If($Lil % $antL -eq 0){
				If ($res.count -gt $antL) {[string]$mQuest="Max første $antL vist."}Else{[string]$mQuest=""}
				$Lil=0
			}
			If($Lix -le $antl){$mQuest+=$r +$EgoCrLf}

			If((($Lix+$Lil+1) -ge $res.count) -or ($Lil % $antL -eq $antL-1)){
				$mQuest+=" "+$EgoCrLf
				$mQuest+="Tryk Yes for at omdøbe til .$repl, tryk No for at slette, eller Cancel for at Liste Rename kommandoer!"
				#Popup box
				$wshell = New-Object -ComObject Wscript.Shell -ErrorAction Stop
				#Start Popup
				If($Lix -lt $antl){
					$rs=$wshell.Popup($mQuest,0,$mHead,3+32)
				}
				For($i=0;$i -lt $antL;$i++){
					If(($Lix+$i) -ge $res.count){Break}
					$ra=$res[$Lix+$i] -split("\.")
					$ra[-1]=$repl
					$rn=$ra -join(".")
					Switch ($rs){
						6	{Rename-Item -Path $res[$Lix+$i] -NewName $rn -force;$retur+=$res[$Lix+$i] + " omdøbt til $rn!"}
						7	{Remove-Item -Path $res[$Lix+$i] -force;$retur+=$res[$Lix+$i] + " Slettet!"}
						#3	{$retur+=$res[$Lix+$i] + " ignoreret!"}
						2	{$retur+="#Rename-Item -Path """+$res[$Lix+$i] +""" -NewName ""$rn"" -force"}
					}
				}
				#If(($Lix+$i) -ge $res.count){Break}
			$Lix+=$antL
		...
		Return $retur
	}Else{ Write-Host "Der er ikke fundet nogen Forekomster af filnavn '$rm1' / '$rm2' ! på drev $drev"}
}
# SIG # Begin signature block
# MIIdAgYJKoZIhvcNAQcCoIIc8zCCHO8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA/hMK5fKDK8xUV
# MNH7d1sj5vQNf0R1atoSm8rah3unQqCCGBMwggT+MIID5qADAgECAhANQkrgvjqI
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIGWDPtqoHykNElcrc6r1Viv1
# K5LE1WDlMSvBmRKCZqTnMA0GCSqGSIb3DQEBAQUABIIBAA6wtbvlMVI4SQrC36O+
# xAEDBbGIXbPvFszEiX7DGAeQAO9VZ3a1q3FBprH7DFTxoviPAsQ+HHJK40lQnjUY
# Bf8U8jco2JE7kcO1wJCUum+6vvWeQAJ4eEHQXGocLSbu8BBoaNpCFCIL/5Cb+9Tj
# w8+lVhsRlDFUWkLKCMIqn+jAZ94oLcvytVpvtGOTvUNQUoOFnNjYVNOJUHW1tdsY
# mntLwD+CfRQt7BSg1tkJ1v0tJ+0foi+3Gwl4Bwv5sWXp04fMhykOKSX2P6BUFELO
# PUHF7yibME0z80HtxnnmreyhnNVFT9LBvRtzZjcQgTkJiKewRcsm7AXhjh18EHch
# c6WhggIwMIICLAYJKoZIhvcNAQkGMYICHTCCAhkCAQEwgYYwcjELMAkGA1UEBhMC
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIyMDExMTE0MTky
# NlowLwYJKoZIhvcNAQkEMSIEIBigWCyj2N9WQln+jTwXwpaqc3XsB9ITCKcyolAJ
# SeBwMA0GCSqGSIb3DQEBAQUABIIBALqWKfwPqCXmPotkAK0I0s4Ji9DEK40syv3j
# a5W4pddjsKWWsVc9d+xliFpsV6rl0/QGO3NZ2zwHegbvge+6AQj3dr2GozTmg50X
# x52MHESVA4GkiqSTGb/oC+8LaALXRN80WgG1s7dEgzF/jqDgzbPqbtgIoU+fkE5P
# 86AUywKcWVi9XywIUW/Jvhr6O7Mv9RiOZYb1ZGcjxS7snsEMITrF7eqVRdD7x288
# UUKiGtTMutKE2r1fB1KItG4M5WXkJ6U55iejDdEctbC/uky0aYzIAg9STQ6nXwYY
# iugv6QTMOPR8BDLl1sSLiQRZfG9TXjSTtAqYNvOy4cHkQZu9Z44=
