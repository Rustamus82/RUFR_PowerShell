<# 
.SYNOPSIS 
New-Check_MK.ps1 installerer nyeste Check_MK agent v. 1.6 , og henter evt. nyeste version af mssql.vbs- plugin i samme folder som scriptet.  
...
# udførende jobnavn: giver variable disk, sti, fil 
$EgoCmd=$PSCommandPath -Split("\\")     # $EgoCmd= 'P:','ps1','SQL-Installation.ps1'
$Egodisk=$EgoCmd[0]
$Egofil=$EgoCmd[-1]
...
$Psti="$Egodisk\$Egosti"
#If($Egosti.length -gt 1){$Psti+="\"}
$IsAdm=[bool](([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -match "S-1-5-32-544")
If(!($IsAdm)) {
    Write-Output "Du skal være logget på som Administrator for at Opdatere Check_MK agent"
    Exit
}
#$Psti


# Variable med navne & lokationer.
$Svbs="mssql.vbs"
$Csti1="install"
$Csti="c:\$Csti1"
If (!(Test-path $Csti\)){New-Item $Csti -ItemType Directory}  # opret c:\Install

$Agent="check_mk_agent.msi"
$Plugsti="C:\ProgramData\checkmk\agent\plugins"
...
$Sqlchk=get-service *sql*		# Er SQL Server installeret
If(!($Sqlchk.count -gt 0)){
	$Svar=Read-Host "Der er ikke installeret SQL på denne server, er dette ok? (J/n)"
	If ($svar.length -gt 0){If($svar.Substring(0,1) -eq "n"){Write-Host "Ok få installeret SQL Server, Farvel"; Exit}}
}Else{Write-Host "Der er installeret SQL på denne server.";Start-Sleep -s 1}

If (test-path $ch16){Write-Host "Check_MK Agent 16 allerede installeret :-) "
	$svar = " "
	do{
		$svar=Read-Host "Vil du fortsætte og installere igen? (ja/nej)"
		If ($svar -in 'ja','j','J','y','Y') {write-host "Fint!";$svar = "ja"
		} elseIf ($svar -in 'Nej','n','N','no') {write-host "ok, slutter her, Farvel!";Exit}
	} until ($svar -eq "ja")
}

If($Sqlchk.count -gt 0){If (!(Test-path $Psti\u\$Svbs)){Write-Host "Check_MK plugin $Svbs er ikke kopieret med over"; exit}}

If (!(Test-path $Psti\$Agent)){Write-Host "ingen adgang til p: drev hvor der er Check_MK -installer: $Agent"; exit}

#If (!(Test-path $Csti\)){Start-Process powershell.exe  -Argument "–NoProfile -Command `"New-Item `"$Csti`" -ItemType `"Directory`"`""  -NoNewWindow -Wait}  # opret c:\Install

If($Sqlchk.count -gt 0){If(!(Test-Path $Csti\$Svbs)){copy $Psti\u\$Svbs $Csti}}

If(!(Test-Path $Csti\$Agent)){copy $Psti\$Agent $Csti}

If(!(Test-Path $Csti\$Agent)){Write-Host "Kopiering af Check_MK agent installationsfil til $Csti fejlede"; exit}

Start-Process -wait -FilePath $Csti\$Agent /norestart

$rc=Get-service check*
If($rc.name.count -gt 0){
	stop-Service check*
	start-Service check*
}
If($Sqlchk.count -gt 0){
	If(!(Test-Path $Plugsti)) {Write-Host "Installationen fejlede, der er ingen folder: $Plugsti til plugin"; exit}

	copy $Psti\u\$Svbs $Plugsti\
	Restart-service check*
}

$rc=Get-Service dosvc*    # ved ikke hvad, men tror det er fra gamel version, optræder kun sjældent
If($rc.name.count -gt 0){
	stop-Service dosvc
	start-Service dosvc
}
...
If (test-path $ch15){Write-Host "Check_MK Agent 15 fundet, den skal af-installeres!"}else{0}
$ch16f=$false
If (test-path $ch16){Write-Host "Check_MK Agent 16 fundet :-) "
	...
	Write-Host " Antal Check_MK firewall regler fundet: "$fwr.name.count
		foreach($f in $fwr){
			...
			If ($chp -eq $ch16){write-host "fwr 16 fundet";$ch16f=$True}
			If ($chp -eq $ch15){write-host "fwr 15 fundet"
			$t="Remove-NetFirewallRule -passthru -Name "
			$t+=$f.name 
			Write-Host $t
			}
		}
		If (!($ch16f)){
			write-Host "opret fw 16"
			...
			Get-NetFirewallrule -DisplayName "Check_MK_Agent_16_(local)"|Get-NetFirewallApplicationFilter|Set-NetFirewallApplicationFilter -Program "C:\Program Files (x86)\checkmk\service\check_mk_agent.exe"
		} 
	}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCsVvEVDLZOZvHK
# 2mQ75IAdivePR/FuczC+rdWBc2R5LaCCGBMwggT+MIID5qADAgECAhANQkrgvjqI
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIKl3T/JA/1zMxAufPbQOPQZ0
# 1LtvCFvljgt4jqX1TeHLMA0GCSqGSIb3DQEBAQUABIIBANnT7KJn4nDp+QGlWB5z
# PgUgYIxBHReDkPWeRTrBF4BIdpMICG/f/GVH0mX7MJ3SPjjEKChZxONO1gkbh9oG
# 9Sc658LERgXxC9PMMTBYqIxThOkXucYHfXRvCt/oTcUdZPjEVol1nFA6VHcEpbpH
# zx1LhIZNZbrXG5IBEqCXqlce5uZ9X4BZ5uQ0YOdJkDJp/lBmufRwP3yS8PXcrbKj
# WY/YMDHIQY37IJJh60dneyv44YQCSw+O+Xa4k7kvW4LSpbuENz8wDWKBULE7CVQN
# quWGD0y/4hKG6j5C/i9s68ingR3HmS2k5Xcz2HthCkHv23PoaQHpOSgJD3M5El6d
# MPShggIwMIICLAYJKoZIhvcNAQkGMYICHTCCAhkCAQEwgYYwcjELMAkGA1UEBhMC
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDcxMjA4MTE1
# MVowLwYJKoZIhvcNAQkEMSIEIGedP5s52mhuqPWldUdjRnRSWKk3gGZRUVsUqZUg
# gYGlMA0GCSqGSIb3DQEBAQUABIIBAGUdC2QUQ5gQHaV+iJEG29PQISUG0vTQ2JQb
# 6Ufc/B9bNvTc2PCH2WvCXELA9WMpsNPE0F8nCx3UkEvrskQcv4Ea5glcqJRfdYTx
# iSKtGWi8Hr/yigGi7f2DlMaD9pQxg0C2hlCO5LFLhyXG5IssK6qhU2YJDkGZMBf1
# fXMR5Y9NjeH8IPzPJFsQEhr7MTIalC2GifVTKJzOHTx/LBjUKLj0aHDLIGLhhNq8
# T1q+z6KG3mpeL/ktm/gG5NjpOrA4g9hOaiHfJvTm7Q1AVaOHQcRL69PRupZF0Byj
# 4CBVj9dcSMWkBGw3DEVVT6Mra1cV6snllvjQXtmtWhBBCVqSYKM=
