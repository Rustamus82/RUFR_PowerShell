$cmd='Get-ADServiceAccount'
$f=0
$rc=get-command
foreach($r in $rc){if($r.name -eq $cmd){$s=$r.Source;$f++}
}
If ($f -gt 0){
    $f=0
    $rc=Get-Module
    foreach($r in $rc){if($r.name -eq $s){$r;$f++}
    }
}
$f


#Installer ad-powershell
$ostype=('xx','Work Station','Domain Controller','NonDC Server')
$os=Get-WmiObject win32_operatingsystem -cn "Localhost" 
If($os.ProductType -lt 2){Write-Host "Kan ikke installere RSAT på en" $ostype[$os.ProductType];Exit}

$IsAdm=[bool](([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -match "S-1-5-32-544")
If(!($IsAdm)) {Write-Host "Der kræves run as administrator for at installere RSAT";exit}

$rc=Get-WindowsFeature -Name *rsat-ad-powershell*
If($rc.InstallState -eq 'Available'){
	Install-WindowsFeature -Name RSAT-AD-PowerShell}  #kun server, kun som admin
ElseIf($rc.InstallState -eq 'Installed'){Write-Host "AD-Modul allerede installeret"}

#$rc=Get-WindowsFeature -Name *ADLDS*
#If($rc.InstallState -eq 'Available'){
#	Install-WindowsFeature -Name ADLDS}  #kun server, kun som admin
#ElseIf($rc.InstallState -eq 'Installed'){Write-Host "ADLDS allerede installeret"}

$rc=Get-WindowsFeature -Name *AD-Domain-Services*
If($rc.InstallState -eq 'Available'){
	Install-WindowsFeature -Name AD-Domain-Services}  #kun server, kun som admin
ElseIf($rc.InstallState -eq 'Installed'){Write-Host "AD-Domain-Services allerede installeret"}

# SIG # Begin signature block
# MIIdAgYJKoZIhvcNAQcCoIIc8zCCHO8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDStNyyWOoaEyZW
# nnofKfnr/fhwaVOk9X30+ibcxVHmsKCCGBMwggT+MIID5qADAgECAhANQkrgvjqI
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIEzcQVIoH+GMORz00wRpnv9J
# MFtJVZmf67YeHY1DZ+nLMA0GCSqGSIb3DQEBAQUABIIBANuGCKFqyclYavxx4fCd
# HKuooY3tHyX5gmAAP1U2wLCa6F+J4bTNAhbvJmQT1jdmufCT0//rzqbixzgdhpR0
# bhapDNj7Coam8//OZw83lv4jFxDO5cd01/uEs2U2LX21PujbwWt12UXRxcD9xxmD
# u7PwM4DpyXxI01SzIvMIl5jPHcz5b70oNtq+QuflFpZ8tOR/eSB74MG3A1tLyluP
# dnU+W+ad1P2VvmAg1bs5p6PaywjF8xAcOaGY/Y9ekX6GOvmh0bdAGl91OOhax01l
# UivkY0JNciCyC2BwRACXcfomT5JWA7skuewwU5qm3g/dZc8L+pfkayNB7k0lgMue
# ShWhggIwMIICLAYJKoZIhvcNAQkGMYICHTCCAhkCAQEwgYYwcjELMAkGA1UEBhMC
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDcxMjA4MDgz
# MFowLwYJKoZIhvcNAQkEMSIEIEuUY+d+Vjb8bRrEMU3jBzMJWZ08uF4rekNnH36J
# ijdJMA0GCSqGSIb3DQEBAQUABIIBAAVk0caOhZ1B/00xA92C77wuGkFyBpwLn8vT
# pJWb5wfcRseAdFLyMv+gbPb1tKE1dCSKo6uSsX5JDJ5/D8Zj/4iYOZQGd2IB0cNM
# JwM4YvnI3LztiCGGTd5FfIgtmmYB4am+dtLD+gt/LjL4YQQJN9CGsuPyDxMexRq5
# 4QJ2zE6W7K1Qjhq1fcO2oQiWLMFhpymVAdyV8WlrFdEKHw0gKTSKwCxLUN3JU4ij
# PatzMA/o1HeOSVT2EOEL5J+u5eAV6xeo0kN8Yyj9ks0mJ+7AaUhlOEPWCo2VyHgR
# iqpzwJIo0ZB+o1z/oa24EMI/OfcegyzVUVP+DJqt5QtW3acXeCY=
