<# 
.SYNOPSIS 
lister Ad brugere efter søge kriterier. 
Script opdateret Mikael Veistrup-Vetlov @ 20210114
Script oprettet Mikael Veistrup-Vetlov @ 20190522


.Parameter user 
AD-Users.ps1 bruger søgeargument <-user> for at søge i AD efter brugere hvor navnet 
...
.DESCRIPTION 
AD-Users.ps1 bruger søgeargument <-user> for at søge i AD <-ad> efter brugere hvor navnet 
indeholder søgeargumentet <-user> og lister brugerne i powershell, 
eller gemmer resultat i dokument: <-gem>  (.log) 
Angiv domæne fra listen: dksund.dk, ssi.ad, sst.dk, sdsp.dk, intdmz.dk, pubdmz.dk, 
- ssidmz01.local, imkit.dk, nsidsdn.dk, dksundtest.dk, lokal = "."

.EXAMPLE 
.\ad-user.ps1 -user mive -ad ssi.ad # -fil usrlist
Søger ad igennem med søgeargument *mive* efter brugere
.EXAMPLE 
Dette kan angives med alias -bruger for -user, -domain/-domæne for -ad #og -gem for -fil
.\ad-user.ps1 -bruger mive -domain ssi.ad #-gem usrlist
.EXAMPLE 
.\ad-user.ps1 -user mive |Format-Table
...
#> 
    ...
    param( 
		...
		HelpMessage="Angiv tekst der skal bruges til at søge efter AD brugere.")] 
#		[ValidateRange(21,65)]
#		[ValidateScript({Test-Path $_ -PathType ‘Container’})]
		...
		HelpMessage="Angiv domæne fra listen: dksund.dk,ssi.ad,sst.dk,sdsp.dk,intdmz.dk,pubdmz.dk,ssidmz01.local,imkit.dk,nsidsdn.dk,dksundtest.dk,lokal ")] 
		[ValidateSet("dksund.dk","ssi.ad","sst.dk","sdsp.dk","intdmz.dk","pubdmz.dk","ssidmz01.local","imkit.dk","nsidsdn.dk","dksundtest.dk",'.','lokal','denne','lokalad')]
        [AllowNull()]
		[Alias("Domæne","Domain")] 
		[string]$ad=""
		)
	Begin{
		$ret=@()
		If($ad -in ("LokalAD",'.','denne','')){$ad=$env:USERDNSDOMAIN}
		$rc=dsquery user -d $ad -name *$user* -limit 1000
		$ret+=$rc
		If (($ret|measure).count -gt 0) {
			Return $ret
		}
	}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAEWLxrBoyVGIeE
# AB6pj8iq0FzHg4xHMkTtv4P0QhxNU6CCGBMwggT+MIID5qADAgECAhANQkrgvjqI
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIDubytaEvVErHRS+cUEkJhqN
# QLbzhh00YjO22AWJh+3CMA0GCSqGSIb3DQEBAQUABIIBADIqCzSJ0GgVEO1HVrNS
# UG3D+J6EPQSCU74nwpuEGh76U+OXfw0eeGO0B9HkENq4NFy0EXcK1PHjh5sPJ2dd
# MSevPi5B0OxFiCvGCN3Y8nK8YsIQY5NuKLJKvKSjJKAVn6F2Gkp8clQYozO5Pmvy
# 5KvsLTD7/TfyFVDqSH6Fo5UjWZ7fvdjM94avsf5NjjdhCx2zjRTpX2o6M7DNcv8/
# NpM+7xjs3mD/D4RXbxVAuUzyF+n072rkpTj1hahGXHKdhHdAmUeXJYdZ5N/mnSKC
# fl0wz8Qi7X1Q3/RA17SWGYKJFTEWzVGjaYvjlnM41/8ADiaXyhffVFhnr11j3klu
# zGWhggIwMIICLAYJKoZIhvcNAQkGMYICHTCCAhkCAQEwgYYwcjELMAkGA1UEBhMC
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMDkwOTE2MDcy
# NVowLwYJKoZIhvcNAQkEMSIEINDg5okwNXS+A6+ebJ67cl/BbiRuj37gXAoiiWAh
# 1qsbMA0GCSqGSIb3DQEBAQUABIIBAIG3pWT2ym9UxxglovZmYpg1UEILObZep4Z4
# uRp4QICL3rq6xMzyc5PO0pbaMsaHN/CqwFsgnqvvr8w522iAh26b2qsQVpvWM7Zr
# ROHIUnp7u9jf71u//QD3/JnOxGBoPzntq+SJC/GAkFf6BPANpkjybNvPz8bZz7vJ
# eJaO5VbDFeBmwkzPiLBTPIG6QmJVKO4QEDf11dvHiJnhjgxzJHSDOc1SPiFptT9G
# yyBIMapubVLUSqthxUNY5+4yiNfbMaYbBMlCuUi01pw1C7n/2Rj01mJdzZvlOSyW
# pAVykO1r8gfjQAm6QEBEP7LM4HhYoncxgO6Mrk3RGMifH194ryk=
