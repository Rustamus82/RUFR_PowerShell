<# 
.SYNOPSIS 
Dette er en funktion for at starte primo SQL Full TSM backup for en SQL Database 
Der skal køres full backup for at TSM kan genkende hver databas og fortsætte med incremential log backup.
Script opdateret Mikael Veistrup-Vetlov @ 20210212
Script oprettet Mikael Veistrup-Vetlov @ 20210212

.Parameter Database 
Tsm-FullDbBk.ps1 bruger argument <-Database> for at starte initial Fuld TSM-Backup for 1 SQL database.
Der kontrolleres IKKE om databasen findes før start.


.DESCRIPTION 
Tsm-FullDbBk.ps1 bruger argument <-Database> for at starte initial Fuld TSM-Backup for 1 SQL database.
Der skal køres full backup for at TSM kan genkende hver databas og fortsætte med incremential log backup.
Kræver Powershell som administrator for at starte jobbet.

.EXAMPLE 
.\Tsm-FullDbBk.ps1 -Database mive-Test 
Der startes Fuld TSM SQL Backup af nævnte database mive-test 
...
#> 
	...
	Param (
		...
		HelpMessage="Angiv <-Database> for at starte initial Fuld TSM-Backup for 1 SQL database.")] 
		[String]$Database
		)
	Begin{
...
$Egosti
		...
		If(!($IsAdm)) {
			Write-Output "Du skal være logget på som Administrator for at Starte Backup på SQL"
			Exit
		}

		$TSMhome="C:\Program Files\Tivoli\TSM\TDPSql"

		If (Test-Path $TSMhome){
			Set-Location c:
			Set-Location $TSMhome
			If(Test-path .\dsm_full.opt){
				$rc=.\tdpsqlc.exe backup $Database full /tsmoptfile=dsm_full.opt #/stripes=4 
			}Else{
				$rc=.\tdpsqlc.exe backup $Database full /tsmoptfile=dsm.opt #/stripes=4 
			}
			If ($rc -match("ACO5460W")){Write-Error "Database $Database findes ikke, Check navn og prøv igen!"}
			$rc
		}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDteisAQLCdhIRV
# tOs7rnQfajW4L6n7rpM9LIyw5E4zC6CCGBMwggT+MIID5qADAgECAhANQkrgvjqI
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIEuZOihxDIqseFQXBZM6r9kZ
# Jy3Bp/EdnghqKyHT1l/YMA0GCSqGSIb3DQEBAQUABIIBADscRfYdkZudiK34ws9a
# 2rKoQkJh/xiCn3Ffig/wm8+lkrsGpIKGVhaXLWqVLuvjavYKdveffer6F4pXlSSu
# AlNzaAg5PNRKhtxyvl0ik5hHQekkHJdBHJmV1Nm26NBDFrkWLE5ddYJ+lxtfOMvm
# LNY+a42jC1v9Wk93ocyny74xgy4gX3n+E9y9e6KJShlf6L+RrUbt8l+mGqzcjhmE
# 3sVyhKXQ8aDz6zhO47qFmofcXmQ64qsaJ1lqYAEH5o0fWpJxAiIfs73qN56KFJzt
# /lmmOn6jTniS58gaCYzXub50pYgIVp4Ot3lo8mB0yCXxCwTgTg6dAczz+VNVOh7C
# MUmhggIwMIICLAYJKoZIhvcNAQkGMYICHTCCAhkCAQEwgYYwcjELMAkGA1UEBhMC
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTIxMTIxNzE3NDAw
# MVowLwYJKoZIhvcNAQkEMSIEICwx73TK9aiQzBoX1eU4QjM3rosZTuNgNTV8fgPu
# 8AvTMA0GCSqGSIb3DQEBAQUABIIBADCAX0dVxpWH7ovVTuoHjXzVm6rznNbQHJnM
# lMImPXIBAr4Cvr1Ixdg5RicX0FutiFR2q4t9CCC74Ec7SpgHcRzLAIggzZdTnWMM
# jXsSwaUFBMCe8oAOpfPkcAqdN9wYKWj67i9T0pjkWL7XwQoWrR3ejxU/TLXWZv5p
# O4Jne8adTe44skdAUimWpllVy1SzSsN9ed61lBUqARNSkQT5itX2tFVcQ51i1Ejl
# fut8+2EOW2m5U4+/K5Z4YJgG8kDN+n3grcIDk20+lQ65As6C87CYCrhPJoxKXCl3
# 734YB0XHd5qg//wHv5tC8VkGVS+m0AvPlawY06gK3TZYVKpaj0Y=
