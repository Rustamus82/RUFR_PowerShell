...
Try {
	...
	[string]$appName = 'SCCM Client Reset'
	...
	[string]$appScriptDate = '02/10/2018'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'RUFR'  -CheckDiskSpace -PersistPrompt -ForceCloseAppsCountdown 300
        ...
        ##Provisioning Mode mode disables
        if((Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\CCM\CcmExec\').ProvisioningMode -eq $true) {
           
           Set-RegistryKey -Key 'HKLM:\SOFTWARE\Microsoft\CCM\CcmExec\' -Name 'ProvisioningMode' -Value 'false'
        }
        ...
        <#Tjek log-filen: 
        & $env:SystemRoot\CCM\Cmtrace.exe "$env:SystemRoot\CCM\Logs\PolicyAgent.log"
        #>
        Start-Sleep 120
        ...
        Start-Sleep 15
        <#Tjek log-filen: 
        & $env:SystemRoot\CCM\Cmtrace.exe "$env:SystemRoot\CCM\Logs\PolicyEvaluator.log" 
        #>
        ...
        Start-Sleep 15
        <#Tjek log-filen: 
        & $env:SystemRoot\CCM\Cmtrace.exe "$env:SystemRoot\CCM\Logs\InventoryProvider.log"
        #>
        ...
        Start-Sleep 15
        <#Tjek log-filen: 
        & $env:SystemRoot\CCM\Cmtrace.exe "$env:SystemRoot\CCM\Logs\StateMessage.log"
        #>

        ##Start CcmEval
        Execute-Process -Path "$envWinDir\CCM\CcmEval.exe"
        <#Tjek log-filen: 
        & $env:SystemRoot\CCM\Cmtrace.exe "$env:SystemRoot\CCM\Logs\CcmEval.log"
        #>

        #Clear-DnsClientCache
        #Start-Sleep 3
        #Register-DnsClient -Confirm
		...
		Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\Software\SDS\SCCM_Client_reset' -Name "Version" -Value $appVersion -Type String
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'RUFR' -ForceCloseAppsCountdown 300
		...
		Set-RegistryKey -Key 'HKEY_LOCAL_MACHINE\Software\SDS\SCCM_Client_reset' -Name "Version" -Value '' -Type String
