function Show-Menu
 {
      ...
      Write-Host "	Tryk D for at sætte mailboks i hold. Dette gøres inden mailboksen slettes. Herved gemmes den uden cost i 12 måneder"
      Write-Host "	Tryk V se mailbokse sat på hold"
      Write-Host
		...
		Set-MsolUserLicense -UserPrincipalName $user@dksund.dk -LicenseOptions $x
		 ...
		 Else {}
              ...
              'D' {
                 ...
                 $stoppet_bruger = ""
                 $videre = "nej"
                 $stoppet_bruger = Read-Host "Tast brugernavn på stoppet bruger:"
                 $videre = Read-Host "Er du sikker på du vil sætte $stoppet_bruger@dksund.dk på hold? ja/(nej)"
                 If ($videre -eq "ja") {
		    ...
		    Write-Host "Mailboksen $stoppet_bruger sat på hold. Mailboksen kan nu slettes via eksisterende procedure".
                  }
                  else {}
                  }
              'V' {
