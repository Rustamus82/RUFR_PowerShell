...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'NoAppToClose' -CheckDiskSpace
		...
		New-Folder -Path "$ProgramFiles\$appVendor\$appName"
		Copy-File -Path "$dirSupportFiles\Smitteopsporing.ico" -Destination "$ProgramFiles\$appVendor\$appName\Smitteopsporing.ico"


        if (Test-Path -Path "$envCommonDesktop\Smitteopsporing.lnk") {Remove-File -Path "$envCommonDesktop\Smitteopsporing.lnk" -ContinueOnError $true} 
        
        New-Shortcut -Path "$envCommonDesktop\Smitteopsporing.lnk" -TargetPath '%ProgramFiles(x86)%\Microsoft\Edge\Application\msedge.exe' -Arguments 'https://coronaopsporing.stps.dk' -IconLocation "$ProgramFiles\$appVendor\$appName\Smitteopsporing.ico" -IconIndex 0 -WorkingDirectory "%ProgramFiles(x86)%\Microsoft\Edge\Application" -WindowStyle Normal
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'NoAppToClose' -ForceCloseAppsCountdown 300
		...
		if (Test-Path -Path "$envCommonDesktop\Smitteopsporing.lnk") {Remove-File -Path "$envCommonDesktop\Smitteopsporing.lnk" -ContinueOnError $true} 
        Remove-File -Path "$ProgramFiles\$appVendor\$appName" -Recurse -ContinueOnError $true
		Remove-Folder -Path "$ProgramFiles\$appVendor\$appName" -ContinueOnError $true
