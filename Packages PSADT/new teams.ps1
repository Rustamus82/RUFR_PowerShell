...
Try {
    ...
    [String]$appVersion = '24152.415.2975.367'
    [String]$appArch = '64-bit'
    [String]$appLang = 'EN'
    [String]$appRevision = '01'
    [String]$appScriptVersion = '1.0.5'
    [String]$appScriptDate = '12/07/2024'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
        ...
        #Show-InstallationWelcome -CloseApps 'Teams' -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
        Show-InstallationWelcome -CloseApps 'NotClosingAnything' -AllowDeferCloseApps -DeferTimes 3 -CheckDiskSpace -ForceCloseAppsCountdown 600
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
        ...
        Show-InstallationWelcome -CloseApps 'Teams,ms-teams' -CloseAppsCountdown 600
        ...
        If ($IsLocalSystemAccount) {
            ...
            foreach ($folder in $UserProfileFolders.ProfilePath) { 
                $settings = Join-Path -Path $folder -ChildPath 'AppData\Local\Microsoft\Teams'
                $settings1 = Join-Path -Path $folder -ChildPath 'AppData\Roaming\Microsoft\Teams'
                New-Item -Path $settings -ItemType Directory -ErrorAction SilentlyContinue
                New-Item -Path $settings1 -ItemType Directory -ErrorAction SilentlyContinue
                Remove-Item -Path "$settings" -Recurse -Force -ErrorAction SilentlyContinue
                Remove-Item -Path "$settings1" -Recurse -Force -ErrorAction SilentlyContinue
            }
        } Else {
            $settings = Join-Path -Path $env:LOCALAPPDATA -ChildPath 'Microsoft\Teams'
            $settings1 = Join-Path -Path $env:APPDATA -ChildPath 'Microsoft\Teams'
    ...
    ElseIf ($deploymentType -ieq 'Repair') {
        ...
        Show-InstallationWelcome -CloseApps 'Teams,ms-teams' -AllowDeferCloseApps -DeferTimes 3 -CheckDiskSpace -ForceCloseAppsCountdown 600
        ...
        #Start classic teams for user.
        #Execute-ProcessAsUser -Path "$envProgramFilesX86\Teams Installer\Teams.exe" -Parameters "–checkInstall"
